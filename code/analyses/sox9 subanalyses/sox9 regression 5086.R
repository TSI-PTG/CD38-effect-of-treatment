...
# load reference data
load("Z:/MISC/Phil/AA All papers in progress/A GC papers/AP1.0A CD38 molecular effects Matthias PFH/data/data_expressionset_k5086.RData")


# DEFINE THE SET ####
set01 <- data_expressionset_k5086[, data_expressionset_k5086$Patient %nin% c(15, 18)]
...
df_exprs <- data_expressionset_k5086[
  featureNames(data_expressionset_k5086) %in% probes_sox9,
  data_expressionset_k5086$Patient %nin% c(15, 18)
] %>%
  exprs() %>%
  t() %>%
...
df_pheno <- data_expressionset_k5086 %>%
  pData %>% 
...
save(resuls_sox9, file = paste(saveDir, "results_sox9_k5086.RData", sep = ""))
...
ggsave(
  filename = paste(saveDir, "Felzartamab SOX9 regression 5086.png"),
