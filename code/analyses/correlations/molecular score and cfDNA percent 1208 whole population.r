...
data_04 <- data_03 %>%
    mutate(
        cor = pmap(
            list(variable, delta, delta_cfdna_percent),
            function(variable, delta, delta_cfdna_percent) {
                df <- delta %>%
                    left_join(
                        delta_cfdna_percent,
                        by = c("Patient", "Felzartamab", "Followup_pairwise"),
                        suffix = c("_score", "_cfdna_percent")
                    ) %>%
                    mutate(variable) %>%
                ...
                df %>%
                    mutate(
                        cor = pmap_dbl(
                            list(data),
                            function(data) {
                                cor(x = data$delta_cfdna_percent, y = data$delta_score, use = "p", method = "spearman")
                            }
                        ),
                        p = pmap_dbl(
                            list(data, cor),
                ...
                df %>%
                    mutate(
                        cor = pmap_dbl(
                            list(data),
                            function(data) {
                                XICOR::xicor(x = data$delta_cfdna_percent, y = data$delta_score, pvalue = FALSE)
                            }
                        ),
                        p = pmap_dbl(
                            list(data),
                            function(data) {
                                XICOR::xicor(x = data$delta_cfdna_percent, y = data$delta_score, pvalue = TRUE)$pval
                            }
                        )
                    )
            }
            ...
            function(variable, delta, delta_cfdna_percent) {
                ...
                df %>% mutate(
                    rq = map(
                        data,
                        function(data) {
                            quantreg::rq(delta_cfdna_percent ~ delta_score, data = data, tau = 0.5, model = TRUE) %>%
                                broom::tidy(se = "nid")
                        }
                    ),
                    rq_slope = map_dbl(
...
save(felzartamab_cfdna_cor_k1208, file = paste(saveDir, "felzartamab_cfdna_percent_cor_k1208_wholepopulation.RData", sep = ""))
