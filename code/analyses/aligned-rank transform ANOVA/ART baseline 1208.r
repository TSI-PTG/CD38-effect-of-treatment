...
data_03 <- data_02 %>%
    mutate(
        art = map(data, art, formula = value ~ Felzartamab),
        art_lm = map(
            art,
            function(art) {
                art %>%
                    artlm(term = "Felzartamab", response = "aligned") %>%
                    summary() %>%
                    pluck("coefficients") %>%
                    as.data.frame() %>%
...
felzartamab_ARTanova_baseline <- data_03
saveDir <- "Z:/MISC/Phil/AA All papers in progress/A GC papers/AP1.0A CD38 molecular effects Matthias PFH/data/"
save(felzartamab_ARTanova_baseline, file = paste(saveDir, "felzartamab_ARTanova_baseline.RData", sep = ""))
