        ...
        table_raw = map(
            ...
            function(table_raw) {
                table_raw %>%
                    ...
                    dplyr::select(AffyID, Symb, logFC, P.Value)
            }
        ),
        probes = map(table_raw, pull, AffyID),
        genes = map(table_raw, pull, Symb)
    )
...
plot_venn_genes <- data_overlap_genes %>%
    mutate(
        venn = pmap(
            list(design, venn_data_genes), function(design, venn_data_genes) {
