        ...
        plot_scatter = pmap(
            ...
            function(data, score) {
                qr <- data %>%
                    ...
                    mutate(
                        ...
                        p = dplyr::case_when(
                            ...
                            TRUE ~ rq_slope_p %>% formatC(digits = 3, format = "f")
                        ),
                ...
                cor <- data %>%
                    ...
                    mutate(
                        ...
                        p = dplyr::case_when(
                            ...
                            TRUE ~ p %>% formatC(digits = 3, format = "f")
                        ),
                        label = paste("scc = ", cor, " | p = ", p, sep = "")
                    ) %>%
                    distinct(Felzartamab, .keep_all = TRUE)
                # null_slope <- mean(c(
                #     min(data$delta_score) / min(data$delta_cfdna_percent),
                #     max(data$delta_score) / max(data$delta_cfdna_percent)
                # ))
                null_slope <- mean(c(
                    min(data$delta_cfdna_percent) / min(data$delta_score),
                    max(data$delta_cfdna_percent) / max(data$delta_score)
                ))
                ...
                plot <- data %>%
                    ...
                    ggplot2::geom_point(pch = 21, size = 3, fill = "grey70") +
                    # ggplot2::geom_text(
