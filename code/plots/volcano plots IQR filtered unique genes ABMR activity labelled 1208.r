...
plot_volcano_panel <- ggarrange(plot_volcano) %>%
    ggpubr::annotate_figure(
        top = text_grob(
            "Genome-wide Effect of Felzartamab Treatment and Relapse Post-Treatment",
            face = "bold.italic", size = 15, hjust = 0.6
        )
    ) %>%
    suppressWarnings()
...
ggsave(
    plot_volcano_panel,
    file = paste(saveDir, "volcano timeseries IQR filtered probes unique genes ABMR activity labelled 1208.png", sep = ""),
    dpi = 300,
    width = 23,
    height = 14,
    units = "cm",
