...
# DEFINE THE SET ####
set <- vienna_1208[featureNames(vienna_1208) %in% features, vienna_1208$STUDY_EVALUATION_ID %nin% c(15, 18)]
df_exprs <- set %>%
    exprs() %>%
    t() %>%
...
df00 <- set %>%
    ...
    expand_grid(category = c("ABMR")) %>%
    dplyr::select(category, Center, CEL, Patient, Group, Felz, TxBx, Group_Felz)  %>% 
...
df_univariate_00 <- df_permanova %>%
            ...
            function(data, features) {
                data %>%
                    ...
                    mutate(
                        ...
                        annotation = "ABMR-related" %>% factor(),
                        .before = 1
                    ) %>%
    ...
    left_join(
        affymap219 %>%
            ...
            dplyr::rename(variable = AffyID, score = Symb),
        by = "variable"
    ) %>%
    relocate(score, .after = variable)
...
ggsave(
    filename = paste(saveDir, "Felzartamab ABMR genes 1208.png"),
    plot = panel_pairs,
    dpi = 600,
    width = 60,
    height = 32,
