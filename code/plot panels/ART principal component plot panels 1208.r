...
panel_violin <- felzartamab_plots %>%
    dplyr::filter(category %>% str_detect("PC")) %>%
    pull(plot_violin) %>%
...
ggsave(
    filename = paste(saveDir, "Felzartamab violin plots PC scores.png"),
...
ggsave(
    filename = paste(saveDir, "Felzartamab patient plots PC scores.png"),
    plot = panel_patient,
    dpi = 300,
    width = 30,
    height = 15,
