...
title_art <- paste("Table i. Non-parametric ANOVA (ART) of estimate cell percentages in biopsies from treated vs untreated patients")
title_art_pairwise <- paste("Table i. Pairwise Non-parametric ANOVA (ART) of estimate cell percentages in biopsies from treated vs untreated patients")
...
data_pairwise_formatted_delta <- felzartamab_ARTanova %>%
    dplyr::select(annotation, score, art_con_interaction_default_tidy, art_con_cld, means_delta) %>%
    unnest(c(art_con_interaction_default_tidy, art_con_cld), names_repair = tidyr_legacy) %>%
    unnest(means_delta, names_repair = tidyr_legacy) %>%
    ...
    mutate(
        delta = paste(
            format(round(mean_delta, 2), nsmall = 1),
            " (",
            round(se_delta, 2),
            ")",
            sep = ""
        ...
        deltadelta = paste(
            format(round(mean_delta_delta, 2), nsmall = 1),
            " (",
            round(se_delta_delta, 2),
            ")",
            sep = ""
        )
    ) %>%
...
title_art_pairwise <- paste("Table i. Mean (se) estimate cell percentages in biopsies from Felzartamab-treated vs placebo-treated patients")
