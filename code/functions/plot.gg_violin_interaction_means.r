gg_violin_interaction <- function(data, variable, score, means_delta, art_con_interaction_default_tidy, patient_label) {
    require(tidyverse)
    require(gghalves)
    require(ggprism)
    require(ggrepel)
    ...
    data <- data %>%
        dplyr::left_join(
            data %>%
                dplyr::reframe(mean = mean(value), .by = c(Followup, Felzartamab, Patient)) %>%
                rstatix::spread(Followup, mean) %>%
                dplyr::mutate(
        ...
        ggplot2::labs(
            title = paste(
                paste(
                    ...
                    delta_delta$mean_delta_delta[[1]] %>% round(2),
                    "| FDR =",
                    delta_delta_p$FDR[[1]]
                ),
                paste(
                    "\u394\u394", delta_delta$Followup_pairwise[[3]], "=",
                    delta_delta$mean_delta_delta[[3]] %>% round(2),
                    "| FDR =",
                    delta_delta_p$FDR[[3]]
                ),
                paste(
                    "\u394\u394", delta_delta$Followup_pairwise[[2]], "=",
                    delta_delta$mean_delta_delta[[2]] %>% round(2),
                    "| FDR =",
                    delta_delta_p$FDR[[2]]
                ),
                sep = "\n"
