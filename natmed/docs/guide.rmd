...
supplementary_table_3 <- data.frame(
    Category = c(
        rep("TCMR-related", 6),
        rep("ABMR-related", 5),
        rep("New ABMR-related PBTs", 4),
        rep("Macrophage-related", 2),
        rep("Recent injury-related", 3),
        rep("Atrophy-fibrosis-related", 2),
        rep("Normal parenchyma", 2)
    ),
    Abbreviation = c(
        "TCMR-RAT", "QCAT", "TCB", "i>1Prob", "t>1Prob", "TCMRProb",
        "NKB", "DSAST", "ABMRProb", "g>0Prob", "ptc>0Prob",
        "AAG", "IIAAG", "NKAAG", "AEG",
        "AMAT1", "QCMAT",
        "IRITD3", "IRITD5", "IRRAT30", 
        "ci>1Prob", "ct>1Prob",
        "KT1", "KT2"
    ),
    Description = c(
        "TCMR-associated (1)", 
        "Cytotoxic T cell associated 2", "T-cell burden related (3)",
...
flextable(supplementary_table_3) %>%
    flextable::set_header_labels(Category = "Category", Abbreviation = "Abbreviation", Description = "Description") %>%
    flextable::add_header_row(values = rep("Supplementary Table 3. Molecular scores (PBTs, classifiers)", flextable::ncol_keys(.))) %>%
    flextable::merge_v() %>%
...
<br>
<span style="font-family: Helvetica; font-size: 20px;">A primary analysis in the present work assessed how felzartamab therapy affected molecular scores related to rejection, ABMR, TCMR, NK cell and T cell burdens, macrophage expression, and injury in patients with ABMR. The study design included two arms, placebo and felzartamab. Patients from both arms were biopsied prior to receiving placebo or felzartamab, and followed up with two additional biopsies at 24 and 52 weeks. The goal was to determine if felzartamab suppressed ABMR. We defined the effect of treatment as the interactive effect between treatment (placebo and felzartamab) and follow up (baseline, week 24, and week 52). This design was chosen to account for the confounding effects of time between placebo and felzartamab. While it is statistically straightforward to measure interactive effects using parametric models, the assumptions of normality and heterogeneity of variance differred across the molecular scores, and thus, for consistency we simply chose to carry out non-parametric tests for all scores. This presented a challenge as standard non-parametric rank-based tests do no allow for testing interactions among independent variables. Thus, we availed to aligned-rank transformation, a technique to rank-order independent variables while maintaining rankings across multiple independent variables. An analysis of variance (ANOVA) can then be carried out on the aligned-rank transformed to allow for fully factorial non-parametric ANOVA. This is powerful in this context because it allows us to test for the interactive effect of treatment (i.e., placebo vs felzartamab) and follow-up  (i.e., change in scores from biopsy to biopsy). Moreover, aligned-rank transformation also allows for us to specify random effects terms in the ANOVA to handle repeated measurements (i.e., biopsies) across patients.</span>
<br>

<span style="font-family: Helvetica; font-size: 20px; ">References:</strong></span>

<span style="font-family: Helvetica; font-size: 15px; font-style: italic;">&#8226; Halloran, P.F., Famulski, K.S. & Reeve, J. Molecular assessment of disease states in kidney transplant biopsy samples. Nat Rev Nephrol 12, 534-548 (2016).</strong></span>
...
## SUMMARIZE RESULTS
<br>
<span style="font-family: Helvetica; font-size: 20px;"> The artANOVA effects are observed to reject or confirm a treatment effect on the interactive effect for each score (i.e., to support the interpretation of post-hoc tests for effect of treatment).</strong></span>
...
```{r, message = FALSE, warning = FALSE, fig.width=85/3.54, fig.height=25/2.54, dpi=300}
...
# Analysing the effect of treatment on injury scores using linear-mixed model {.tabset}


## INTRODUCTION

<span style="font-family: Helvetica; font-size: 20px;">While ABMR-related scores changed monotonically over time, dipping at week 24 then rising again by week 52, initial observations of injury scores over time alluded to a different relationship. Injury scores appeared to gradual increase overtime in the placebo arm and a gradual decrease overtime in the felzartamab arm. However, the use of artANOVA, a non-parametric rank-based, test failed to detect such a relationship. This led us to ask if estimating the slopes of injury curves over time would be a more informative interrogation of these data. We applied a linear mixed-model to answer this question. </span>
<br>

<span style="font-family: Helvetica; font-size: 20px; ">References:</strong></span>

<span style="font-family: Helvetica; font-size: 15px; font-style: italic;">&#8226; Vonesh, E., et al. Mixed-effects models for slope-based endpoints in clinical trials of chronic kidney disease. Stat Med 38, 4218-4239 (2019).</strong></span>
...
```{r, message = FALSE, warning = FALSE, fig.width=8, fig.height=12, dpi=300}
...
```{r, message = FALSE, warning = FALSE, fig.width=15, fig.height=10, dpi=300, message = FALSE, warning = FALSE}
...
<span style="font-family: Helvetica; font-size: 20px;">In addition to molecular scores, we had the opportunity to carry out genome-wide assessement of the effect of felzartamab. We use the Linear Models for Microarray Data (limma) pipeline for applying empirical Bayes mediated t-tests on individual genes. Specific contrasts to assess the interaction between treatment and time are specified to estimate the effect of treatment on each gene. </span>
<br>
